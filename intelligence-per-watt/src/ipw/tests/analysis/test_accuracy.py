...
def test_zero_energy_imputed_from_power_and_latency(
    ...
    # Avg latency = 2.0
    # Imputed energy for zero reading = 9.0; combined energies = [10.0, 9.0] => avg 9.5
    assert result.summary["accuracy"] == pytest.approx(0.5)
    assert result.summary["avg_per_query_energy_joules"] == pytest.approx(9.5)
    assert result.summary["intelligence_per_joule"] == pytest.approx(0.5 / 9.5)
    assert result.summary["intelligence_per_watt"] == pytest.approx(0.5 / 4.5)
    eff = result.data["efficiency"]["test-model"]["energy"]
    assert eff["imputed_from_power"] == pytest.approx(9.0)
